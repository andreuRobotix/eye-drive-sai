...
# The library refuses to connect unless the hub's RPC version matches the
# package's EXACTLY. Our SAI reports (1, 0, 67) while the package expects
# (1, 0, 73) -- same major.minor, only the build differs, so the motor message
# formats are compatible. We don't want to flash firmware, so we replace
# RPC_VERSION with a sentinel that compares equal to any version, which makes
# that check pass. (Remove this block to restore the strict check.)
class _AnyVersion:
    ...
    def __repr__(self):
        return "<any RPC version (firmware check bypassed)>"
